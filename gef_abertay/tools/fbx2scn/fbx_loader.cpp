...
void AddMesh(FbxNode& node, Scene& scene, Platform& platform)
{
	...
	FBXSDK_printf("Exporting mesh: %s\n", node.GetName());
	...
	if( !fbx_mesh->IsTriangleMesh() )
	{
		...
		fbx_mesh = (FbxMesh*)converter.Triangulate(fbx_mesh, true);
	}
	...
	// build the vertex buffer
	{
		...
		{
			total_byte_size = vertex_buffer_size*sizeof(gef::Mesh::SkinnedVertex);
			vertices = (Mesh::SkinnedVertex*)malloc(total_byte_size);
			vertex_buffer_data.vertex_byte_size = sizeof(gef::Mesh::SkinnedVertex);
		}
		else
		{
			total_byte_size = vertex_buffer_size*sizeof(gef::Mesh::Vertex);
			vertices = (Mesh::Vertex*)malloc(total_byte_size);
			vertex_buffer_data.vertex_byte_size = sizeof(gef::Mesh::Vertex);
		}

		memset(vertices, 0xff, total_byte_size);
		...
		int bytes_written = vertex_data - vertices;
		_ASSERTE(bytes_written == total_byte_size);
